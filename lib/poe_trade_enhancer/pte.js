...
(function() {
  ...
  var path_of_economy_auth_token = "<<#sed:poe_auth_token>>";
  var path_of_economy_url = "<<#sed:poe_url>>";
...
var saveSearch = function() {
  ...
  var saveForm = function() {
    try {
      ...
      postSearchToDatabase(searches[name])
      close();
    } catch(ex) {
...
var initSaveManager = function() {
  ...
  /* Path Of Economy addons */
  var postSearchToDatabase = function(search) {
      var searchClone = Object.assign({}, search);
      var query = translateQuery(searchClone.query)

      searchClone.query = query;
      console.log('SAVING', searchClone);

      var data = { 
        search: searchClone, 
        id: app.searchId,
        poe_auth_token: path_of_economy_auth_token,
      };
      fetch(path_of_economy_url + "api/v1/analyses/save", {
          mode: 'cors',
          method: 'POST',
          headers: {
   ...
   var translateQuery = function(query) {
      var queryClone = Object.assign({}, query);
      var stats = queryClone.stats.map(function(stat) {
          var statClone = Object.assign({}, stat);

          var filters = statClone.filters.map((function(filter) {
              filter.data = app.static_.knownStatsFlat[filter.id];
              return filter;
          }));
          statClone.filters = filters;
          return statClone;
      });

      queryClone.stats = stats;
      return queryClone;
  };
})();